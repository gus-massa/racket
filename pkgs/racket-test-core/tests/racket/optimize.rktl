...
;Test special cases for hash constructors
(let ()
  (define (test-hash-constructor h use-equal?)
    (test-comp `(lambda () (,h) 1)
               `(lambda () 1)
               #t)
    (test-comp `(lambda (k v) (,h k v) 1)
               `(lambda (k v) 1)
               (not use-equal?))
    (test-comp `(lambda (k v) (,h k v 3 4) 1)
               `(lambda (k v) 1)
               (not use-equal?)))
  (test-hash-constructor hash #t) ; k may be a struct with a custom equal+hash
  (test-hash-constructor hasheq #f)
  (test-hash-constructor hasheqv #f))
(let ()
  (define (test-make-hash-null make-h)
    (test-comp `(lambda () (,make-h null) 1)
               `(lambda () 1))
    (test-comp `(lambda () (,make-h) 1)
               `(lambda () 1)))
  (test-make-hash-null 'make-hash)
  (test-make-hash-null 'make-immutable-hash)
  (test-make-hash-null 'make-weak-hash)
  (test-make-hash-null 'make-hasheq)
  (test-make-hash-null 'make-immutable-hasheq)
  (test-make-hash-null 'make-weak-hasheq)
  (test-make-hash-null 'make-hasheqv)
  (test-make-hash-null 'make-immutable-hasheqv)
  (test-make-hash-null 'make-weak-hasheqv))
