...
; Test move after drop of repeated variable
(test-comp '(lambda (f x) (let ([r (f)])
                            (if r x r)))
           '(lambda (f x) (if (f) x #f)))
(test-comp '(lambda (f x) (let ([r (f)])
                            (not (if r r x))))
           '(lambda (f x) (not (if (f) #t x))))
(test-comp '(lambda (f x) (not (let ([r (f)])
                                 (if r r x))))
           '(lambda (f x) (not (if (f) #t x))))
(test-comp '(lambda (f x) (not (let ([r (f)])
                                 (if (if r r x) (x 1) (x 2)))))
           '(lambda (f x) (not (if (if (f) #t x) (x 1) (x 2)))))
(test-comp '(lambda (f x) (let ([r (f)])
                            (if (pair? (cons 1 2)) 'ok (error r))
                            r))
           '(lambda (f x) (if (pair? (cons 1 2)) 'ok 'ignored)
                          (values (f))))
(test-comp '(lambda (f x) (let ([r (f)])
                            (if (box? (cons 1 2)) (error r) 'ok)
                            r))
           '(lambda (f x) (if (box? (cons 1 2)) 'ignored 'ok)
                          (values (f))))
(test-comp '(lambda (f x) (let ([r (f)])
                            (if (pair? (cons 1 2)) r (error r))))
           '(lambda (f x) (if (pair? (cons 1 2)) (values (f)) 'ignored)))
(test-comp '(lambda (f x) (let ([r (f)])
                            (if (box? (cons 1 2)) (error r) r)))
           '(lambda (f x) (if (box? (cons 1 2)) 'ignored (values (f)))))

(test-comp '(lambda () (let ([r (box 0)])
                            (list (car (cons 1 r)) r)))
           '(lambda () (list 1 (box 0))))
(test-comp '(lambda () (let ([r (box 0)])
                         (list (cdr (cons r 2)) r)))
...
(test-comp '(lambda (x) (let ([r (box x)])
                          (box r)
                          r))
           '(lambda (x) (box x)))
(test-comp '(lambda (x) (let ([r (box x)])
