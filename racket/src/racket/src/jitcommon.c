...
static int common10(mz_jit_state *jitter, void *_data)
{
  ...
  /* prim_result_arity_code */
  /* R0 has primitive */
  {
    GC_CAN_IGNORE jit_insn *ref_nofx, *ref_slow, *ref_multiple;
    GC_CAN_IGNORE jit_insn *refr USED_ONLY_FOR_FUTURES;
    
    sjc.prim_result_arity_code = jit_get_ip();
    mz_prolog(JIT_R2);
    ...
    jit_pusharg_i(JIT_R0);
    __END_SHORT_JUMPS__(1);
    mz_finish_prim_lwe(ts_scheme_primitive_result_arity, refr);
    __START_SHORT_JUMPS__(1);
    jit_retval(JIT_R0);
    jit_addi_p(JIT_RUNSTACK, JIT_RUNSTACK, WORDS_TO_BYTES(1));
    JIT_UPDATE_THREAD_RSPTR();
    ...
    ref_multiple = jit_bmsi_i(jit_forward(), JIT_R2, SCHEME_PRIM_IS_MULTI_RESULT);
    ...
    scheme_jit_register_sub_func(jitter, sjc.prim_result_arity_code, scheme_false);
    CHECK_LIMIT();
  }
