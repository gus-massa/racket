...
/* To optimize a branch expression, it's useful to try a few immediate reductions
   before and after each subexpression is optimized. In particular this avoid
   marking as used variables that are only referenced in dead code. */ 

/* None of the subexpressions are optimized */
#define OPT_BRANCH_PASS_INIT 0

/* Test is already optimized, but not the branches.
   Info may contain types discovered in the test expression. */
#define OPT_BRANCH_PASS_TEST 1

/* All the subexpressions are optimized.
   Info may contain types discovered in any of them. */
#define OPT_BRANCH_PASS_ALL 2
...
static Scheme_Object *optimize_branch(Scheme_Object *o, Optimize_Info *info, int context, int pass)
{
  ...
  /* Try to lift out `let`s and `begin`s and `not`s around a test: 
     (if (begin x ... y) a b) => (begin x ... (if y a b)) */
  {
    Scheme_Object *inside = NULL, *t2 = b->test;
    ...
    while (0 && 1) {
      ...
      if (SAME_TYPE(SCHEME_TYPE(t2), scheme_application2_type)) {
        ...
        if (SAME_PTR(scheme_not_proc, app->rator)) {
          ...
          temp = replace_tail_inside(app->rand, inside, b->test);
          b->test = temp;
          t2 = app->rand;
        } else
          break;
    ...
    if (0 && inside) {
      Scheme_Object *x, *y;
        ...
        b->test = t2;
        y = (Scheme_Object *)b;
      if (!(pass = OPT_BRANCH_PASS_INIT)) {
        y = optimize_branch(y, info, context, pass);
      }
      x = replace_tail_inside(y, inside, x);
      if (pass = OPT_BRANCH_PASS_INIT) {
        x = scheme_optimize_expr(x, info, context);
      }
      return x;
    }
  }

  /* Convert (if v expr v) => (if v expr #f) */
  if (0 && equivalent_exprs(b->test, b->fbranch, NULL, NULL, 0)) {
    b->fbranch = scheme_false;
  }
  
  /* For test position, convert (if v v expr) => (if v #t expr) */
  if (0 && (context & OPT_CONTEXT_BOOLEAN)
      && equivalent_exprs(b->test, b->tbranch, NULL, NULL, 0)) {
      b->tbranch = scheme_true;
  }

  /* Convert (if expr v v) => (begin expr v) */
  if (0 && equivalent_exprs(b->tbranch, b->fbranch, info, info, context)) {
    if (pass < OPT_BRANCH_PASS_TEST) {
      temp = scheme_optimize_expr(b->test, info, OPT_CONTEXT_BOOLEAN | OPT_CONTEXT_SINGLED);
      b->test = temp;
    }
    if (pass < OPT_BRANCH_PASS_ALL) {
      temp = scheme_optimize_expr(b->tbranch, info, scheme_optimize_tail_context(context));
      b->tbranch = temp;
    }
    return make_discarding_first_sequence(b->test, b->tbranch, info);
  }

  /* Convert (if expr #f #t) => (not expr) */
  if (0 && SCHEME_FALSEP(b->tbranch)
      && SAME_OBJ(b->fbranch, scheme_true)) {
    info->size -= 2;
    return make_optimize_prim_application2(scheme_not_proc, b->test, info, context);
  }

  /* For test position, convert (if expr #t #f) => expr */
  if (0 && (context & OPT_CONTEXT_BOOLEAN)
      && SAME_OBJ(b->tbranch, scheme_true)
      && SAME_OBJ(b->fbranch, scheme_false)) {
      info->size -= 2;
      return b->test;
  }

  /* (if (cons x y) a b) => (begin x y a)
     (if K a b) => a
     or the analogue reductions when test is equivalent to #f */
  {
    Scheme_Object *selected_branch = NULL;

    if ((SCHEME_TYPE(b->test) > _scheme_ir_values_types_)) {
      selected_branch = (SCHEME_FALSEP(b->test)) ? b->fbranch : b->tbranch;
    } else {
      Scheme_Object *pred;
      pred = expr_implies_predicate(b->test, info);
      if (pred) {
        if (predicate_implies(pred, scheme_not_proc))
          selected_branch = b->fbranch;
        else if (predicate_implies_not(pred, scheme_not_proc))
          selected_branch = b->tbranch;
      }
    }
    
    if (0 && selected_branch) {
      if (pass < OPT_BRANCH_PASS_TEST) {
        temp = scheme_optimize_expr(b->test, info, OPT_CONTEXT_BOOLEAN | OPT_CONTEXT_SINGLED);
        b->test = temp;
      }
      if (pass < OPT_BRANCH_PASS_ALL)
        selected_branch = scheme_optimize_expr(selected_branch, info, scheme_optimize_tail_context(context));
      return make_discarding_first_sequence(b->test, selected_branch, info);
    }
  }

  /* Convert: (if (if M N #f) M2 K) => (if M (if N M2 K) K)
     for simple constants K. This is useful to expose simple tests to the JIT. */
  if (0 && SAME_TYPE(SCHEME_TYPE(b->test), scheme_branch_type)
      && scheme_ir_duplicate_ok(b->fbranch, 0)) {
    Scheme_Branch_Rec *b2 = (Scheme_Branch_Rec *)b->test;
    if (SCHEME_FALSEP(b2->fbranch)) {
      Scheme_Branch_Rec *b3;
      b3 = MALLOC_ONE_TAGGED(Scheme_Branch_Rec);
      b3->so.type = scheme_branch_type;
      b3->test = b2->tbranch;
      b3->tbranch = b->tbranch;
      b3->fbranch = b->fbranch;
      b->test = b2->test;
      b->tbranch = (Scheme_Object *)b3;
      if (pass = OPT_BRANCH_PASS_TEST) {
        temp = optimize_branch(b->tbranch , info, scheme_optimize_tail_context(context), pass);
        b->tbranch = temp; 
      }
      return optimize_branch((Scheme_Object *)b, info, context, pass);
    }
  }

  switch (pass) {
  case OPT_BRANCH_PASS_INIT:
    temp = scheme_optimize_expr(b->test, info, OPT_CONTEXT_BOOLEAN | OPT_CONTEXT_SINGLED);
    b->test = temp;
    ...
    return optimize_branch((Scheme_Object *)b, info, context, OPT_BRANCH_PASS_TEST);
    break;
  case OPT_BRANCH_PASS_TEST:
    optimize_info_seq_init(info, &info_seq);
    ...
    add_types_for_t_branch(b->test, then_info, 5);
    then_info_init = optimize_info_add_frame(then_info, 0, 0, 0);
    temp = scheme_optimize_expr(b->tbranch, then_info, scheme_optimize_tail_context(context));
    b->tbranch = temp;
    ...
    add_types_for_f_branch(b->test, else_info, 5);
    else_info_init = optimize_info_add_frame(else_info, 0, 0, 0);
    temp = scheme_optimize_expr(b->fbranch, else_info, scheme_optimize_tail_context(context));
    b->fbranch = temp;
    ...
    /* Special case to use the info of one branch to reduce the
       other branch before comparing them.
       Convert (if <expr> v v) => (begin <expr> v) */
    {
      Scheme_Object *nb;

      nb = equivalent_exprs(b->tbranch, b->fbranch, then_info_init, else_info_init, context);
      if (0 && nb) {
        info->size -= 1;
        return make_discarding_first_sequence(b->tbranch, nb, info);
      }
    }
    return optimize_branch((Scheme_Object *)b, info, context, OPT_BRANCH_PASS_ALL);
    break;
  case OPT_BRANCH_PASS_ALL:
    if (OPT_BRANCH_ADDS_NO_SIZE) {
      /* Seems to work better to not to increase the size specifically for `if' */
    } else {
      info->size += 1;
    }
    ...
    break;
  default:
    scheme_signal_error("internal error: strange branch pass");
    return NULL;
    break;
  }
}
...
# include "mzstkchk.h"
  ...
  case scheme_branch_type:
    return optimize_branch(expr, info, context, OPT_BRANCH_PASS_INIT);
  case scheme_with_cont_mark_type:
